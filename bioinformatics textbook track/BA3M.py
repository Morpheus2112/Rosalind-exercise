# -*- coding: utf-8 -*-
"""
Created on Wed Mar 14 09:57:30 2018
...
#fname = 'rosalind_ba3l.txt'
#lines = list(l for l in open(fname))
#k = int(lines[0].split(' ')[0])
#d = int(lines[0].split(' ')[1])
#pairs = map(lambda e: (e.split('|')[0].strip(), e.split('|')[1].strip()), lines[1:])
#genome = pair_genome_path(k,d,pairs)
#with open(fname+'.out.txt', "w") as f:
#        f.write(genome)
